...
class BackupHandler:
    def __init__(self):
        ...
        logging.basicConfig(level=logging.INFO, filename='%s/log/%s-result.log' % (sys.path[0], self._backuptime_string))
    ...
    def sendmail(self):
        if self._globalconfig.mailmode:
            ...
            mailsender.send('--- 设备配置备份 --- %s ---' % self._backuptime_string,
                            plaincontent, 'temp/%s.tar.gz,log/%s-pexpect.log,log/%s-result.log'
                            % (self._backuptime_string, self._backuptime_string, self._backuptime_string))
        else:
